...
if (!isset($_SESSION['loggedIn'])) {
    ...
    if (isset($_POST["updateMedicine"])) {
        ...
        //Collect the variables from the Medicine 
        ...
        if ($statement = $con->prepare($query)) {
            if ($statement->bind_param("sisisisi", $medicineName, $dosageQty, $dosageUnit, $milligramQty, $milligramUnit, $frequencyQty, $frequencyUnit, $medicine_id)) {
                ...
                if ($statement->execute()) {
                    echo "It has Successfully Updated";
                    header("location: index.php");
                } else {
                    echo "Try again...Update Error!";
                    header("location: edit_drug.php?id=" . $medicine_id. "");
                }
            } else {
                echo "Rectify the error binding";
            }
        } else {
            echo "Error in sql query statement";
        }
...
<div class="container">
    <div class="row mt-4">
        <div class="col-md-8 offset-md-2">
            <h4 class="display-5 my-1">Edit and Update Medicine</h4>
            <?php
            if (!empty($row)) {
                echo '
